...
class avTemplate
{
	...
	*/
	...
	function avTemplate( $root_dir = './', $unknowns = 'keep')
	{
	...
	*/
	function set_var($var_name, $var_value)
	{
		...
		{
			while (list($key,$value) = each($var_value))
			{
			...
			if($loop_code != ''){
					...
					if (is_array($loop_ar))
					{
						...
						{	
							...
							while ( list($k, $v) = each($loop_ar[$ar_keys[$i]]))
							{
